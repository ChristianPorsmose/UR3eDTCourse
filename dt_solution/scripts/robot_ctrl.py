...
from communication.typed_protocol import LoadProgram, Play, MsgProtocol, InjectStuckJoint, InjectWear, LoadTCPProgram
...
def main():
    ...
    with TypedRabbitMQClient(Rabbitmq(**config)) as typed_client:
        ...
        while True:
            ...
            # control_queue.put(
            #     LoadTCPProgram(
            #         tcp_position=[ 0.0, 0.0,  0.4 ],
            #         tcp_rotation=[0.0, 0, 0.0],
            #         max_velocity=50,
            #         acceleration=70
            #     )
            # )
            ...
            #     wear_injected = True
            #     print("Injected wear")
            # if not stuck_injected and elapsed >= 400:
            #     inject_stuck_joints()
            #     stuck_injected = True
